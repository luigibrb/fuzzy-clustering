distn <- # Function computing distance between A = (Am, Al, Ar) and B = (Bm, Bl, Br)
  function (A, B, lambda = 0.5, ro = 0.5)
  {
    Am <- A[,1]
    Al <- A[,2]
    Ar <- A[,3]
...
Tn <- # Bootstrap statitic
  function (dat, ind)
  {
    ...
    stat <- n * (sum(distn(Y_tilde_hat,Y_tilde_avg)) / sum(distn(Y_tilde,Y_tilde_avg)))
    return(stat)
  }
